...
1. **Elevation strategy**: admin user -> self-relaunch elevated with guard `-ElevatedFor` (aborts if UAC elevates as a DIFFERENT admin account; per-user installs would land on the wrong profile). The relaunch is gated on `$isAdmin -and -not $ElevatedFor`: `IsInRole(Administrator)` reflects the EFFECTIVE token, so the relaunched child is still "admin" — without the `-not $ElevatedFor` check the script relaunches itself infinitely (bug found on real run, fixed). Standard user -> no elevation; admin-only items are disabled (shown `[✗]` dark red, Space no-op) via `NeedsAdmin`/`Disabled` properties on catalog rows.
